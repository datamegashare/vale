// ═══════════════════════════════════════════════════════════════
// Vale Digital — Backend GAS v1.0  (Etapa 1)
...
function doGet(e) {
  ...
  try {
    switch (accion) {
      ...
      default             : return respError('Acción GET desconocida: ' + accion);
    }
  } catch (err) {
    return respError('Error interno GET [' + accion + ']: ' + err.message);
  }
}
...
function doPost(e) {
  ...
  try {
    switch (accion) {
      case 'guardarVale'  : return acGuardarVale(body);
      case 'eliminarVale' : return acEliminarVale(body);
      default             : return respError('Acción POST desconocida: ' + accion);
    }
  } catch (err) {
    return respError('Error interno POST [' + accion + ']: ' + err.message);
  }
}
...
function acEliminarVale(body) {
  const { id_vale, usuario_email } = body;
  if (!id_vale)       return respError('id_vale requerido.');
  if (!usuario_email) return respError('usuario_email requerido.');
