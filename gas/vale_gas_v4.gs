// ═══════════════════════════════════════════════════════════════
// Vale Digital — Backend GAS v4.0  (Etapa 3 — Panel Almacén/Pañol)
// Endpoints v2: login, guardarVale, getMisVales, eliminarVale
// Endpoints v3: getPendientesAprobacion, aprobarVale,
//               rechazarVale, getValesAprobados
// Endpoints v4: getValesPorGestionar, getValesPendientes,
...
// GET ?accion=getTodosVales&email=xxx&solicitante=xxx (opcional, filtra por nombre)
// Solo JEFE_OBRA — retorna todos los vales de su obra, últimos 30 días.
function acGetTodosVales(params) {
  const email = (params.email || '').trim().toLowerCase();
  if (!email) return respError('Email requerido.');

  const jefe = buscarUsuario(email);
  if (!jefe)        return respError('Usuario no registrado.');
  if (!jefe.activo) return respError('Usuario inactivo.');
  if (jefe.rol !== 'JEFE_OBRA') return respError('Solo el JEFE_OBRA puede consultar todos los vales.');
  ...
  let resultado = vales.filter(v => {
    if (v.obra_codigo !== jefe.obra_codigo) return false;
    if (v.eliminado === true || v.eliminado === 'TRUE') return false;
    const fecha = new Date(v.fecha_hora);
    return fecha >= hace30;
  });
